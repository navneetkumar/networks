...
public abstract class ConsoleStart implements NetBaseInterface {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String debug = configMgr.getProperty("test.test.test");
			...
			String initialAppName = theNetBase.loadInitialApp("console.initialapp");
			if ( initialAppName != null ) theNetBase.startInitialApp(initialAppName);
			theNetBase.shutdown(); // we're done when the initial app terminates
			
			// if there was no initial app, just run as a daemon
			// 1/23/13 -- this is out of date.  There are no apps loaded at all if there is no inital app.
			// There must always be an initial app.
