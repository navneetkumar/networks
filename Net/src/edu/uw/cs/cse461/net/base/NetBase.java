...
public abstract class NetBase implements NetBaseInterface {
	...
	// Used in the version() method below, and also used by code distributed
	// as jar's, in their main()'s (e.g., AbstractTester.java).
	public static final String VERSION_STRING = "1.2.1 1/25/2013";
	...
	public String version() {
		// Keep in sync with test jar (main() in AbstractTester.java, which is currently in package consoleapps.grading).
	...
	protected NetBase(ConfigManager configMgr) {
		...
		// Load and start the initial application
		//_loadApps();
	}
	...
	/**
	 * Loads the initial , as specified by a config file field (e.g., console.initialapp).
   * @param fieldName Probably useless generality - caller can decide what the config field name to use should be.
   * @return On success, the loadable app name of the initial app.  On failure, null.
	 */
	public String loadInitialApp(String fieldName) {
    String intialAppClass = NetBase.theNetBase().config().getProperty(fieldName);
    if (intialAppClass == null) {
      Log.e(TAG, "Config file bug -- No " + fieldName + " entry.");
      return null;
    }
    //if ( intialAppClass.equals("none")) return null;  // we now require an inital application
    try {
      return loadApp(intialAppClass);
    } catch (Exception e) {
      Log.e(TAG,  "Couldn't load inital application class " + intialAppClass + ": " + e.getMessage());
    }
    return null;
   ...
   * @return false if there's no initial app; true otherwise
	 */
	public boolean startInitialApp(String initialAppName) {
    try {
